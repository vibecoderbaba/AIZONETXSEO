...
function generateDynamicKeywordData(seed: string, country: string) {
  ...
  const brandName = seed.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
  
  // 1. Detect Niche Category based on seed string
  let category: "B2B_TECH" | "MONEY_FINANCE" | "LOCAL_SERVICE" | "GENERAL" = "GENERAL";
  
  if (/\b(seo|marketing|crm|software|analytics|api|platform|tool|dev|design|saas|app|ai|tech|cloud|automation)\b/i.test(seedLower)) {
    category = "B2B_TECH";
  } else if (/\b(money|earn|income|finance|invest|stock|crypto|business|passive|cash|jobs|affiliate|side hustle)\b/i.test(seedLower)) {
    category = "MONEY_FINANCE";
  } else if (/\b(real estate|cleaning|plumber|lawyer|clinic|dental|restaurant|hotel|salon|service|agent|agency|consultant)\b/i.test(seedLower)) {
    category = "LOCAL_SERVICE";
  }

  // 2. Select highly natural, non-repetitive, actual search-demand query blueprints
  let blueprints: Array<{ phrase: string; intent: string; volMultiplier: number; diff: number; cpc: number }> = [];

  if (category === "MONEY_FINANCE") {
    blueprints = [
      { phrase: `how to ${seedLower}`, intent: "educational", volMultiplier: 2.2, diff: 32, cpc: 1.10 },
      { phrase: `${seedLower} from home`, intent: "informational", volMultiplier: 1.8, diff: 28, cpc: 1.50 },
      { phrase: `best ways to ${seedLower}`, intent: "comparative", volMultiplier: 1.5, diff: 36, cpc: 2.40 },
      { phrase: `passive ${seedLower} ideas`, intent: "informational", volMultiplier: 1.3, diff: 45, cpc: 3.10 },
      { phrase: `legit ${seedLower} apps`, intent: "commercial", volMultiplier: 1.1, diff: 40, cpc: 2.90 },
      { phrase: `${seedLower} for students`, intent: "educational", volMultiplier: 0.9, diff: 22, cpc: 0.80 },
      { phrase: `side hustles to ${seedLower}`, intent: "problem solving", volMultiplier: 1.2, diff: 34, cpc: 1.90 },
      { phrase: `fastest way to ${seedLower}`, intent: "buyer intent", volMultiplier: 0.8, diff: 48, cpc: 2.20 },
      { phrase: `earn ${seedLower} typing jobs`, intent: "transactional", volMultiplier: 0.7, diff: 25, cpc: 0.95 },
      { phrase: `make 100 a day online`, intent: "transactional", volMultiplier: 1.4, diff: 55, cpc: 1.80 }
    ];
  } else if (category === "B2B_TECH") {
    blueprints = [
      { phrase: `best ${seedLower} software`, intent: "comparative", volMultiplier: 2.0, diff: 48, cpc: 4.80 },
      { phrase: `top ${seedLower} tools`, intent: "commercial", volMultiplier: 1.6, diff: 42, cpc: 3.50 },
      { phrase: `free ${seedLower} templates`, intent: "informational", volMultiplier: 1.4, diff: 24, cpc: 0.60 },
      { phrase: `${seedLower} automation workflow`, intent: "problem solving", volMultiplier: 1.1, diff: 38, cpc: 5.20 },
      { phrase: `${seedLower} pricing comparison`, intent: "buyer intent", volMultiplier: 0.8, diff: 31, cpc: 6.80 },
      { phrase: `how to implement ${seedLower}`, intent: "educational", volMultiplier: 0.9, diff: 29, cpc: 2.10 },
      { phrase: `open source ${seedLower} alternative`, intent: "comparative", volMultiplier: 0.7, diff: 35, cpc: 1.90 },
      { phrase: `enterprise ${seedLower} platform`, intent: "buyer intent", volMultiplier: 1.2, diff: 54, cpc: 8.50 },
      { phrase: `${seedLower} tutorial for beginners`, intent: "educational", volMultiplier: 1.3, diff: 26, cpc: 1.30 },
      { phrase: `best ${seedLower} for small business`, intent: "commercial", volMultiplier: 1.5, diff: 40, cpc: 5.90 }
    ];
  } else if (category === "LOCAL_SERVICE") {
    const loc = country.split(',')[0].trim();
    blueprints = [
      { phrase: `best ${seedLower} in ${loc}`, intent: "transactional", volMultiplier: 1.8, diff: 29, cpc: 4.50 },
      { phrase: `${seedLower} services near me`, intent: "buyer intent", volMultiplier: 2.2, diff: 35, cpc: 5.80 },
      { phrase: `affordable ${seedLower} packages`, intent: "buyer intent", volMultiplier: 1.2, diff: 25, cpc: 3.90 },
      { phrase: `licensed ${seedLower} company`, intent: "commercial", volMultiplier: 0.9, diff: 28, cpc: 6.20 },
      { phrase: `${seedLower} consultation price`, intent: "transactional", volMultiplier: 0.7, diff: 22, cpc: 4.10 },
      { phrase: `top rated ${seedLower} experts`, intent: "commercial", volMultiplier: 1.0, diff: 31, cpc: 5.50 },
      { phrase: `how to choose a ${seedLower}`, intent: "educational", volMultiplier: 0.8, diff: 18, cpc: 1.50 },
      { phrase: `${seedLower} checklist for home`, intent: "informational", volMultiplier: 1.1, diff: 15, cpc: 0.80 },
      { phrase: `commercial ${seedLower} solutions`, intent: "buyer intent", volMultiplier: 0.6, diff: 42, cpc: 7.20 },
      { phrase: `verified ${seedLower} reviews`, intent: "comparative", volMultiplier: 0.5, diff: 20, cpc: 2.30 }
    ];
  } else {
    blueprints = [
      { phrase: `best ${seedLower} guide`, intent: "educational", volMultiplier: 1.8, diff: 32, cpc: 1.20 },
      { phrase: `how to use ${seedLower}`, intent: "educational", volMultiplier: 2.0, diff: 25, cpc: 0.90 },
      { phrase: `free ${seedLower} resources`, intent: "informational", volMultiplier: 1.5, diff: 18, cpc: 0.40 },
      { phrase: `${seedLower} vs alternatives`, intent: "comparative", volMultiplier: 1.2, diff: 34, cpc: 2.10 },
      { phrase: `what is ${seedLower}`, intent: "educational", volMultiplier: 2.5, diff: 20, cpc: 0.50 },
      { phrase: `${seedLower} examples and cases`, intent: "informational", volMultiplier: 1.1, diff: 28, cpc: 1.40 },
      { phrase: `top ${seedLower} tools`, intent: "commercial", volMultiplier: 1.3, diff: 38, cpc: 2.80 },
      { phrase: `easy ${seedLower} strategy`, intent: "problem solving", volMultiplier: 1.0, diff: 30, cpc: 3.20 },
      { phrase: `advanced ${seedLower} tutorial`, intent: "educational", volMultiplier: 0.8, diff: 42, cpc: 1.80 },
      { phrase: `latest ${seedLower} trends`, intent: "informational", volMultiplier: 1.4, diff: 26, cpc: 1.50 }
    ];
  }

  // 3. Construct exact, non-repetitive keywords
  const keywords = blueprints.map(b => {
    const volume = Math.round((Math.random() * 2000 + 1000) * b.volMultiplier);
    const difficulty = b.diff;
  ...
  return {
    summary: { totalVolume, avgDifficulty, opportunityCount },
    clusters: [
      {
        name: `${brandName} Search Landscape`,
      ...
      {
        name: `Informational & Educational Contexts`,
    ...
    contentOpportunities: {
      blog: [
        `10 Best ${brandName} Strategies to Skyrocket Growth in 2026 (Expert Review)`,
        `How to Master ${brandName} for Small Businesses and Startups`,
        `The Step-by-Step ${brandName} Tutorial for Absolute Beginners`
      ],
      youtube: [
        `The Truth About ${brandName} (Don't Buy Until You Watch This!)`,
        `I Tried 5 Different ${brandName} Methodologies — Here is the Best One`,
        `${brandName} Comparison Guide: ChatGPT vs Claude vs Custom Systems`
      ],
      social: [
        `Why 82% of businesses are failing their ${brandName} implementations — and how to fix it in 3 steps.`,
        `The exact keyword cluster roadmap we used to 10x our organic sessions.`,
        `Is generic ${brandName} dead? Why semantic authority is the future of organic rankings.`
      ],
      instagram: [
        `Reels Hook: "Stop wasting hours on ${brandName} manual work! Here's the AI cheat code... 🤫"`,
        `Carousel concept: "5 Secret ${brandName} tools the top 1% use to dominate organic rankings."`,
        `Reels Hook: "I spent 48 hours testing every ${brandName} software so you don't have to. Here's the winner 🏆"`
      ],
      faq: [
        `What are the most cost-effective ${brandName} platforms for local startups in ${country}?`,
        `How do I run a semantic comparison between different ${brandName} methodologies?`,
        `Is there a free template or sitemap structure I can use to index ${brandName} terms quickly?`
      ]
    },
    seoRecommendations: {
      quickWins: [
...
### 🎯 Cluster 1: Core Semantic ${brandName} Solutions
...
### 📚 Cluster 2: ${brandName} Informational & Educational
...
1.  **"10 Best ${brandName} Strategies to Skyrocket Growth in 2026"**
2.  **"The Step-by-Step ${brandName} Tutorial for Absolute Beginners"**

### 🎥 High-Click YouTube Video Hooks:
- *"I Tried 5 Different ${brandName} Methodologies — Here is the Best One"* (Promising transparent comparative insights to capture pre-purchase leads).

### 💼 High-Authority LinkedIn Thought Leadership:
- *"Why 82% of businesses are failing their ${brandName} implementations — and the 3 semantic shifts that solve it."*
